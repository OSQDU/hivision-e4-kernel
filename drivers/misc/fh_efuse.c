...
//	printk(
//			"app before raw info is: entry: %x,,key size: %x, trans_key_start: %x, no is : %x, src_key_add:%x\n",
//			efuse_info.efuse_entry_no, efuse_info.key_size,
//			efuse_info.trans_key_start_no, efuse_info.trans_key_size,(u32)efuse_info.key_buff);
//
//
	...
	//	memcpy((void*)s_efuse_obj.status.protect_bits,(void*)efuse_info.status.protect_bits,sizeof());
		//mdelay(300);
		break;
...
//	printk(
//			"app after raw info is: entry: %x,,key size: %x, trans_key_start: %x, no is : %x, src_key_add:%x\n",
//			efuse_info.efuse_entry_no, efuse_info.key_size,
//			efuse_info.trans_key_start_no, efuse_info.trans_key_size,(u32)efuse_info.key_buff);


	memcpy((void*)&s_efuse_obj.status,(void*)&efuse_info.status,sizeof(struct efuse_status));

	if (copy_to_user((void __user*) (&((EFUSE_INFO* )arg)->status), (void*) &efuse_info.status,
...
//	printk(
//			"########raw info is: entry: %x,,key size: %x, trans_key_start: %x, no is : %x, src_key_add:%x,pro_data0:%x,pro_data1:%x\n",
//			t_efuse_info.efuse_entry_no, t_efuse_info.key_size,
//			t_efuse_info.trans_key_start_no, t_efuse_info.trans_key_size,(u32)t_efuse_info.key_buff,t_efuse_info.status.protect_bits[0],t_efuse_info.status.protect_bits[1]);
