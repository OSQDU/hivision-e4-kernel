 ...
 * @note: misc audio driver for linbao embedded audio codec.
 * @note History:
...
#define ACW_DACR_FIFO 					0xf0a00300
...
#ifndef CONFIG_MACH_LINBAO_FPGA
	int reg;
    if (capture == type)
    {
...
int register_tx_dma(struct fh_audio_cfg  *audio_config)
{
    ...
    tx_config =  kzalloc(sizeof(struct dma_slave_config), GFP_KERNEL);
    if (!tx_config)
    {
        return -ENOMEM;
    }
    ...
    tx_config->slave_id = ACW_HW_NUM_TX;
    tx_config->dst_maxburst = FH_DMA_MSIZE_8;
    tx_config->src_maxburst = FH_DMA_MSIZE_8;
    tx_config->dst_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
    tx_config->device_fc = TRUE;
    tx_config->dst_addr = ACW_DACL_FIFO;
...
int register_rx_dma( struct fh_audio_cfg  *audio_config)
{
    ...
    rx_config =  kzalloc(sizeof(struct dma_slave_config), GFP_KERNEL);
    if (!rx_config)
    {
        return -ENOMEM;
    }
    rx_config->slave_id = ACW_HW_NUM_RX;
    rx_config->dst_maxburst = FH_DMA_MSIZE_8;
    rx_config->src_maxburst = FH_DMA_MSIZE_8;
    rx_config->src_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
    rx_config->device_fc = TRUE;
    rx_config->src_addr = ACW_ADC_FIFO;
    ...
    status = readl( fh_audio_module.regs + ACW_CTRL);//clear rx fifo
    status =  status|(1<<29);
    writel(status,fh_audio_module.regs + ACW_CTRL);
    writel(0x10025,fh_audio_module.regs + ACW_RXFIFO_CTRL);/*enable rx fifo*/
...
void playback_start_wq_handler(struct work_struct *work)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            writel(0x110025, fh_audio_module.regs + ACW_TXFIFO_CTRL); //enable tx fifo read enable
            avail = readl( fh_audio_module.regs + ACW_CTRL);//clear rx fifo
            avail =  avail|(1<<30)|(1<<31);
            writel(avail,fh_audio_module.regs + ACW_CTRL);
    ...
    switch (cmd)
    {
        case AC_INIT_CAPTURE_MEM:
            ...
            if (0 == arg_config_support(&cfg))
            {
                ...
                i2c_write_codec(0x0, 0x11);
                i2c_write_codec(0x1, 0x5);
                i2c_write_codec(0x1f, 0x1);
                i2c_write_codec(0x2, 0x10);
                /*  * config wrapper work format  *   */
                writel(0x1331, fh_audio_module.regs + ACW_DBG_CTL);//set work format
                writel(0x0, fh_audio_module.regs + ACW_DAT_CTL);//set work format
        ...
        case AC_INIT_PLAYBACK_MEM:
            ...
            if (0 == arg_config_support(&cfg))
            {
                ...
                if(!config_flag){
					...
					/*  * config wrapper work format  *   */
	                writel(0x1331, fh_audio_module.regs + ACW_DBG_CTL);//set work format
	                writel(0x000, fh_audio_module.regs + ACW_DAT_CTL);//set work format
	                /** dont config again **/
	                config_flag = 1;
...
static int __devinit fh_audio_drv_probe(struct platform_device *pdev)
{
    ...
    fh_audio_module.regs = devm_ioremap(&pdev->dev, mem->start, resource_size(mem));
