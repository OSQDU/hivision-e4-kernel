...
void sd_int_hdl(PADAPTER padapter)
{
	...
	#define T_START 0
	#define T_END 3
	struct timeval t_array1[T_END],td1;
	if ((padapter->bDriverStopped == _TRUE) ||
	    (padapter->bSurpriseRemoved == _TRUE))
		return;
	do_gettimeofday(&t_array1[0]);
#ifdef CONFIG_USING_CMD52_READ_INT
	...
	do_gettimeofday(&t_array1[2]);
	if (t_array1[T_END - 1].tv_usec - t_array1[T_START].tv_usec >=0 ){
		td1.tv_usec = t_array1[T_END - 1].tv_usec - t_array1[T_START].tv_usec;
		td1.tv_sec = t_array1[T_END - 1].tv_sec - t_array1[T_START].tv_sec;
	}else{
		td1.tv_usec = t_array1[T_END - 1].tv_usec + 1000000 - t_array1[T_START].tv_usec;
		td1.tv_sec =  t_array1[T_END - 1].tv_sec - t_array1[T_START].tv_sec - 1;
	}
	/*
	if ((td1.tv_sec > 0) || (td1.tv_usec > 300000)){
		int i;
		for (i=0;i<T_END;i++){
			printk("\n[%s]t%d %us %uus",__func__,i,t_array1[i].tv_sec,t_array1[i].tv_usec);
		}
		printk("\n[%s]td %us %uus\n",__func__,td1.tv_sec,td1.tv_usec);
	}
	*/
}
