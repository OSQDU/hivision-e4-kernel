...
wlan_get_info_stats(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
	...
	if (ret == MLAN_STATUS_SUCCESS)
		ret = MLAN_STATUS_PENDING;
#else
	PRINTM(MMSG, "[%s-%d] GIVE UP HostCmd_CMD_802_11_GET_LOG HostCmd_ACT_GEN_GET\n", __func__, __LINE__);
...
start_config:
	...
	if (pioctl_req->action == MLAN_ACT_SET) {
		...
		case MLAN_OID_SCAN_NORMAL:
			PRINTM(MMSG, "[%s-%d] begin wlan_scan_networks(puser_scan_in NULL)\n", __func__, __LINE__);
		...
		case MLAN_OID_SCAN_USER_CONFIG:
			PRINTM(MMSG, "[%s-%d] begin wlan_scan_networks(...)\n", __func__, __LINE__);
...
wlan_ops_sta_ioctl(t_void *adapter, pmlan_ioctl_req pioctl_req)
{
	...
	ENTER();
	PRINTM(MMSG, "[%s %s-%d] pioctl_req->req_id 0x%8x\n", __FILE__, __func__, __LINE__, pioctl_req->req_id);
