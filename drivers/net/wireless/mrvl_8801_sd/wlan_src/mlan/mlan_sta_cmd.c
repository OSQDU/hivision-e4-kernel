			...
			t_u16 cmd_action, void *pdata_buf)
{
	...
	if (smc->action == HostCmd_ACT_GEN_SET) {
		...
		for (i = 0; i < smc->addrCount && i < MAX_ADDR_FILTER; i++) {
			...
			paddr_tlv->filtertype = smc->addr[i].filterType;
			ptlv_buffer +=
				(paddr_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			// pcmd->size += (paddr_tlv->header.len +
			// sizeof(MrvlIEtypesHeader_t));
			paddr_tlv->header.len =
		...
		/**channel width*/
				pchanlist_tlv->chan_scan_param[i].radio_type |=
					(smc->chanlist[i].chanwidth << 2);
		/**second off channel*/
				chan2Offset =
				...
				pchanlist_tlv->chan_scan_param[i].
					min_scan_time =
					smc->chanlist[i].minScanTime;
				pchanlist_tlv->chan_scan_param[i].
					max_scan_time =
					smc->chanlist[i].maxScanTime;
			}
			...
			ptlv_buffer +=
				(pchanlist_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			// pcmd->size += (pchanlist_tlv->header.len +
			// sizeof(MrvlIEtypesHeader_t));
			pchanlist_tlv->header.len =
		...
		if (smc->filterCount) {
			...
			ptlv_buffer +=
				(pfilter_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
//            pcmd->size += (pfilter_tlv->header.len + sizeof(MrvlIEtypesHeader_t));
			pfilter_tlv->header.len =
		...
		if (smc->beaconPeriod) {
			...
			pbcnofchan_tlv->beacon_of_chan =
				smc->beaconOfChan | (smc->beaconOfChan << 1);
			ptlv_buffer +=
				sizeof(MrvlIEtypesHeader_t) +
				pbcnofchan_tlv->header.len;
		}
		...
		if (smc->ie_len) {
			...
			ptlv_buffer +=
				(pie_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			// pcmd->size += (pfilter_tlv->header.len +
			// sizeof(MrvlIEtypesHeader_t));
			pie_tlv->header.len =
			 ...
			 IN t_void *pdata_buf, IN t_void *pcmd_buf)
{
	...
	PRINTM(MMSG, "[%s-%d] cmd_no 0x%4x cmd_action 0x%x\n", __func__, __LINE__, cmd_no, cmd_action);
	/* Prepare command */
	switch (cmd_no) {
