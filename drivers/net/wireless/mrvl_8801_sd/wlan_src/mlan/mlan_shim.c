...
/**
 *  @brief queue rx_work (only called by mlan_main_process())
 *
 *  @param pmadapter	A pointer to mlan_adapter structure
 *
 *  @return			N/A
	...
	)
{
	...
	while (MTRUE) {
		...
		PRINTM(MMSG, "[%s-%d] one dequeue, the remaining rx_pkts_queued: %d\n", __func__, __LINE__, pmadapter->rx_pkts_queued);
	...
	do {
		...
		if (pmadapter->sdio_ireg) {
			...
			if (pmadapter->data_received && pmadapter->rx_work_flag) {
				PRINTM(MMSG, "[%s-%d]begin mlan_queue_rx_work() DUE to data_received\n", __func__, __LINE__);
				mlan_queue_rx_work(pmadapter);
			}
		}
		...
		if (pmadapter->cmd_resp_received) {
			PRINTM(MMSG, "[%s-%d]pmadapter->cmd_resp_received==1\n", __func__, __LINE__);
		...
		if (pmadapter->event_received) {
			PRINTM(MMSG, "[%s-%d]pmadapter->event_received==1\n", __func__, __LINE__);
		...
		PRINTM(MMSG, "[%s-%d]pmadapter->hw_status %d cmd_sent %u\n", __func__, __LINE__, pmadapter->hw_status, pmadapter->cmd_sent);
