...
wlan_process_event(pmlan_adapter pmadapter)
{
	...
	PRINTM(MMSG, "[%s-%d]event_cause 0x%8x pmadapter->hw_status %d\n", __func__, __LINE__, pmadapter->event_cause, pmadapter->hw_status);
		 ...
		 IN t_u32 cmd_oid, IN t_void *pioctl_buf, IN t_void *pdata_buf)
{
	...
	PRINTM(MMSG, "[%s %s-%d] (cmd_no 0x%4x cmd_action 0x%x) ingored!!! \n", __FILE__, __func__, __LINE__, cmd_action);
        return MLAN_STATUS_SUCCESS;
	default:
	     break;
     }
			     ...
			     IN cmd_ctrl_node *pcmd_node, IN t_u32 add_tail)
{
	...
	PRINTM(MMSG, "[%s-%d][begin util_enqueue_list_xx()]command 0x%4x add_tail %d pmadapter->hw_status %d\n",
	    __func__, __LINE__, command, add_tail, pmadapter->hw_status);
...
wlan_process_cmdresp(mlan_adapter *pmadapter)
{
	...
	PRINTM(MCMND,
	       ...
	       resp->seq_num);
	PRINTM(MMSG, "[%s-%d]CMD_RESP (%lu.%06lu): 0x%4x, result %d, len %d, seqno 0x%x\n",
	       __func__, __LINE__, sec, usec, orig_cmdresp_no, cmdresp_result, resp->size, resp->seq_num);
...
#endif
	PRINTM(MMSG, "[%s-%d]pmadapter->ps_mode %d pmadapter->init_auto_ds %d pmadapter->hw_status %d\n",
	    __func__, __LINE__, pmadapter->ps_mode, pmadapter->init_auto_ds, pmadapter->hw_status);
#if defined(STA_SUPPORT)
		     ...
		     IN HostCmd_DS_COMMAND *resp, IN t_void *pioctl_buf)
{
	...
	PRINTM(MFATAL, "[%s-%d]fw_cap_info=0x%x, dev_cap_mask=0x%x\n",
	       __func__, __LINE__, wlan_le32_to_cpu(hw_spec->fw_cap_info),
	...
	PRINTM(MMSG, "[%s-%d] begin wlan_set_regiontable() \n", __func__, __LINE__);
