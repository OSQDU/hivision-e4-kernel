			      ...
			      IN t_u8 filtered_scan)
{
	...
	ENTER();
	PRINTM(MMSG, "[%s-%d]NELEMENTS(pmadapter->region_channel): %d\n", __func__, __LINE__, NELEMENTS(pmadapter->region_channel));
	for (region_idx = 0;
		...
		if (wlan_11d_is_enabled(pmpriv) &&
			...
			PRINTM(MMSG, "[%s-%d]universal_channel[%d].valid=0\n", __func__, __LINE__, region_idx);
			    continue;
			}
			pscan_region =
			...
			if (!pmadapter->region_channel[region_idx].valid) {
			    PRINTM(MMSG, "[%s-%d]region_channel[%d].valid=0 (pmpriv->state_11d.enable_11d: %d pmpriv->media_connected: %d)\n",
					__func__, __LINE__, region_idx, pmpriv->state_11d.enable_11d, pmpriv->media_connected);
			    continue;
			}
		...
		if (puser_scan_in && !puser_scan_in->chan_list[0].chan_number &&
			...
			if (!radio_type && (pscan_region->band != BAND_B) &&
			    ...
			    PRINTM(MMSG, "[%s-%d]radio_type=0 && pscan_region->band: %d\n", __func__, __LINE__, pscan_region->band);
			    continue;
			}
		}
		PRINTM(MMSG, "[%s-%d]pmpriv->config_bands:%d pmadapter->adhoc_start_band: %d pscan_region->band: %d\n",
		    __func__, __LINE__, pmpriv->config_bands, pmadapter->adhoc_start_band, pscan_region->band);
		if (!wlan_is_band_compatible
		    (pmpriv->config_bands | pmadapter->adhoc_start_band,
		     pscan_region->band)) {
			PRINTM(MMSG, "wlan_is_band_compatible() return fail\n");
			continue;
		}
		PRINTM(MMSG, "[%s-%d]pscan_region->num_cfp %d\n", __func__, __LINE__, pscan_region->num_cfp);
		for (next_chan = 0;
			...
			pscan_chan_list[chan_idx].chan_number =
				(t_u8)cfp->channel;
			PRINTM(MMSG, "pscan_chan_list[%d].chan_number: %d Type: %s cfp->max_tx_power: %d\n", chan_idx, (t_u8)cfp->channel,
				scan_type == MLAN_SCAN_TYPE_PASSIVE ? "Passive" : "Active", cfp->max_tx_power);
			    ...
			    OUT t_u8 *pscan_current_only)
{
	...
	*pscan_current_only = MFALSE;
	...
	if (puser_scan_in) {
		...
		PRINTM(MMSG, "[%s-%d]puser_scan_in->chan_list[0].chan_number: %d\n", __func__, __LINE__, puser_scan_in->chan_list[0].chan_number);
	} else {
		PRINTM(MMSG, "[%s-%d]puser_scan_in is NULL\n", __func__, __LINE__);
	...
	*ppchan_list_out = (MrvlIEtypes_ChanListParamSet_t *)ptlv_pos;
	...
	if (puser_scan_in && puser_scan_in->chan_list[0].chan_number) {
		...
		PRINTM(MMSG, "[%s-%d]Scan: Using supplied channel list\n", __func__, __LINE__);
		...
		for (chan_idx = 0;
			...
			PRINTM(MMSG, "[%s-%d](pscan_chan_list + %d)->chan_number: %d\n", __func__, __LINE__, (pscan_chan_list + chan_idx)->chan_number);
			radio_type =
		...
		PRINTM(MMSG, "Scan: Creating full region channel list\n");
		   ...
		   IN const wlan_user_scan_cfg *puser_scan_in)
{
	...
	PRINTM(MMSG, "[%s-%d]pscan_chan_list->chan_number: %d\n", __func__, __LINE__, pscan_chan_list->chan_number);
...
wlan_queue_scan_cmd(IN mlan_private *pmpriv, IN cmd_ctrl_node *pcmd_node)
{
	...
	ENTER();
	PRINTM(MMSG, "[%s-%d][begin util_enqueue_list_tail()] pmadapter->hw_status %d\n", __func__, __LINE__, pmadapter->hw_status);
	if (pcmd_node == MNULL)
		goto done;
			...
			IN t_void *pioctl_buf, IN mlan_802_11_ssid *preq_ssid)
{
	...
	PRINTM(MMSG, "[%s-%d] begin wlan_scan_networks(chan_number: %d)\n", __func__, __LINE__, pscan_cfg->chan_list[0].chan_number);
