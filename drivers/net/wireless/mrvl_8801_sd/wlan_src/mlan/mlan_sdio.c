...
wlan_sdio_init_ioport(mlan_adapter *pmadapter)
{
	...
	PRINTM(MMSG, "[%s-%d] SDIO FUNC1 IO port: 0x%x\n", __func__, __LINE__, pmadapter->ioport);
...
wlan_prog_fw_w_helper(IN pmlan_adapter pmadapter, t_u8 *fw, t_u32 fw_len)
{
	...
	PRINTM(MMSG, "[%s-%d]Wlan: FW download over, firmwarelen=%d downloaded %d\n",
	       __func__, __LINE__, firmwarelen, offset);
...
/**
 *  @brief This function gets interrupt status. pmadapter->sdio_ireg |= sdio_ireg;
 *
 *  @param pmadapter    A pointer to mlan_adapter structure
 *  @return             N/A
 */
...
wlan_interrupt(pmlan_adapter pmadapter)
{
	...
	if (MLAN_STATUS_SUCCESS !=
		...
		PRINTM(MERROR, "[%s-%d]moal_read_data_sync: read registers failed\n", __func__, __LINE__);
...
wlan_process_int_status(mlan_adapter *pmadapter)
{
	...
	PRINTM(MMSG, "[%s-%d]pmadapter->sdio_ireg 0x%x\n", __func__, __LINE__, pmadapter->sdio_ireg);
...
wlan_set_sdio_gpio_int(IN pmlan_private priv)
{
	...
	if (pmadapter->int_mode == INT_MODE_GPIO) {
		...
		PRINTM(MMSG, "[%s-%d]SDIO_GPIO_INT_CONFIG: interrupt mode is SDIO\n", __func__, __LINE__);
	}
