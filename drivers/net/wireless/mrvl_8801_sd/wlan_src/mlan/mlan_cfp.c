...
wlan_set_regiontable(mlan_private *pmpriv, t_u8 region, t_u8 band)
{
	...
	ENTER();
	PRINTM(MMSG, "[%s-%d] region%d band%d\n", __func__, __LINE__, region, band);
	...
	PRINTM(MMSG, "[%s-%d] region_channel[0].valid%d region_channel[0].region%d region_channel[0].num_cfp%d\n",
	    __func__, __LINE__, pmadapter->region_channel[0].valid, pmadapter->region_channel[0].region, pmadapter->region_channel[0].num_cfp);
	LEAVE();
	return MLAN_STATUS_SUCCESS;
}
