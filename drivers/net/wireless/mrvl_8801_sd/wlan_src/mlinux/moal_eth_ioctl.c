		  ...
		  t_u8 wait_option)
{
	...
	PRINTM(MMSG, "[%s-%d] cmd_header.command 0x%4x\n", __func__, __LINE__, woal_le16_to_cpu(cmd_header.command));
...
woal_priv_setgetautoassoc(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	if (strlen(respbuf) ==
	    (strlen(CMD_MARVELL) + strlen(PRIV_CMD_AUTOASSOC))) {
		PRINTM(MERROR, "No argument, invalid operation!\n");
		ret = -EINVAL;
		goto done;
	} else {
	...
	if (user_data_len == 1) {
		...
		if (data[0] == AUTO_ASSOC_TYPE_FW_RECONN) {
			...
			if (user_data_len == 5) {
				...
				data[4] = 0;	/* Default fw auto re-connect
						   flags */
			}
			req = woal_alloc_mlan_ioctl_req(sizeof
							(mlan_ds_misc_cfg));
	...
	if (strncmp(respbuf, "channel", strlen("channel")) == 0) {
		...
		if ((user_data_len != 0) && (user_data_len % 4 == 0)) {
			...
			for (i = 0; i < smc->chan_num; i++) {
				smc->chanlist[i].chan_number = data[i * 4];
				smc->chanlist[i].chanwidth = data[i * 4 + 1];
				smc->chanlist[i].minScanTime = data[i * 4 + 2];
				smc->chanlist[i].maxScanTime = data[i * 4 + 3];
			}
		} else {
			PRINTM(MERROR, "Invalid number of args!\n");
			ret = -EINVAL;
		...
		if ((user_data_len != 0) && (user_data_len % 13 == 0)) {
			...
			for (i = 0; i < smc->addrCount; i++) {
				memcpy(smc->addr[i].startaddr, &data[i * 13],
				       MLAN_MAC_ADDR_LENGTH);
				memcpy(smc->addr[i].endaddr, &data[i * 13 + 6],
				       MLAN_MAC_ADDR_LENGTH);
