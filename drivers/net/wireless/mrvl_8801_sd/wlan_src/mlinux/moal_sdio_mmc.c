...
woal_sdio_interrupt(struct sdio_func *func)
{
	...
	PRINTM(MMSG, "[%s-%d] irq_handler is called\n", __func__, __LINE__);
...
woal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	...
	PRINTM(MMSG, "[%s-%d]vendor=0x%4.04X device=0x%4.04X class=%d function=%d\n",
	       __func__, __LINE__, func->vendor, func->device, func->class, func->num);
		    ...
		    t_u32 timeout)
{
	...
	if (!status) {
		...
		PRINTM(MERROR, "[%s-%d]cmd53 read error=%d\n", __func__, __LINE__, status);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0)
		/* issue abort cmd52 command through F0 */
		sdio_f0_writeb(((struct sdio_mmc_card *)handle->card)->func,
			       0x01, SDIO_CCCR_ABORT, &status);
...
woal_bus_register(void)
{
	...
	if (sdio_register_driver(&wlan_sdio)) {
		PRINTM(MFATAL, "[%s-%d]SDIO Driver Registration Failed \n", __func__, __LINE__);
		LEAVE();
