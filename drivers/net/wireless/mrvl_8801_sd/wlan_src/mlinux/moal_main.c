...
#else
#define DEFAULT_DEBUG_MASK	(MFATAL | MERROR)
#endif /* DEBUG_LEVEL2 */
...
/**
 *  @brief This function dynamically populates the driver mode table(handle->drv_mode)
 *
...
/**
 *  @brief This function initializes software(handle->pmlan_adapter)
 *
 *  @param handle   A pointer to moal_handle structure
 *
 *  @return         MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
...
woal_init_sw(moal_handle *handle)
{
	...
	PRINTM(MMSG, "device.rx_work=%d cpu_num=%d\n", device.rx_work,
	       num_possible_cpus());
	PRINTM(MMSG, "[%s-%d]mfg_mode%u drvdbg0x%x max_tx_buf%u ps_mode%u\n",
	    __func__, __LINE__, device.mfg_mode, device.drvdbg, device.max_tx_buf, device.ps_mode);
	...
	if (MLAN_STATUS_SUCCESS == mlan_register(&device, &pmlan)) {
		...
		ret = MLAN_STATUS_FAILURE;
		PRINTM(MMSG, "[%s-%d] mlan_register() fail\n", __func__, __LINE__);
	}
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 32)
			 ...
			 t_u8 wait_option)
{
	...
	PRINTM(MMSG, "[%s-%d] size %u wait_option %u\n", __func__, __LINE__, size, wait_option);
...
woal_add_card_dpc(moal_handle *handle)
{
	...
	PRINTM(MMSG, "[%s-%d]wlan: version = %s\n", __func__, __LINE__, str_buf);
...
#ifdef MFG_CMD_SUPPORT
	if (mfg_mode == MLAN_INIT_PARA_ENABLED) {
	    PRINTM(MMSG, "[%s-%d] mfg_mode MLAN_INIT_PARA_ENABLED, goto done\n", __func__, __LINE__);
            goto done;
	}
#endif
...
woal_init_fw_dpc(moal_handle *handle)
{
	...
	if (handle->firmware) {
		...
		PRINTM(MMSG, "[%s-%d]WLAN FW is active\n", __func__, __LINE__);
	}

	/** Cal data request */
	memset(&param, 0, sizeof(mlan_init_param));
	PRINTM(MMSG, "[%s-%d]cal_data_cfg: %s\n", __func__, __LINE__, cal_data_cfg);
...
woal_request_fw_dpc(moal_handle *handle, const struct firmware *firmware)
{
	...
	if (!firmware) {
		...
		PRINTM(MMSG, "[%s-%d]begin woal_request_fw()\n", __func__, __LINE__);
...
woal_request_fw(moal_handle *handle)
{
	...
	PRINTM(MMSG, "[%s-%d]Request firmware: %s(req_fw_nowait %d)\n", __func__, __LINE__, handle->drv_mode.fw_name, req_fw_nowait);
	if (req_fw_nowait) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 32)
		...
		for (i = 0;
			...
			if (priv->auto_assoc_priv.auto_assoc_trigger_flag ==
				...
				if (priv->auto_assoc_priv.
					...
					woal_disconnect(priv, MOAL_CMD_WAIT,
							NULL);
					if (priv->auto_assoc_priv.
...
woal_add_card(void *card)
{
	...
	PRINTM(MMSG, "[%s-%d] set m_handle[%d]\n", __func__, __LINE__, index);
...
#if 1
	char mac_tmp[20];
	PRINTM(MMSG, "[%s-%d] mac_addr %s handle->mac_addr %s\n", __func__, __LINE__, mac_addr, mac_addr_s(handle->mac_addr, mac_tmp));
#endif /* modify by luoc */
...
woal_init_module(void)
{
	...
	PRINTM(MMSG, "[%s-%d]wlan: Loading MWLAN driver\n", __func__, __LINE__);
	...
	if (ret == MLAN_STATUS_SUCCESS)
		PRINTM(MMSG, "[%s-%d]wlan: Driver loaded successfully\n", __func__, __LINE__);
	else
		PRINTM(MMSG, "[%s-%d]wlan: Driver loading failed\n", __func__, __LINE__);
