	...
	)
{
	...
	else
	{
		...
		if(retryCount == 0)  // no retry in the last 2-second duration
		{
			...
			if(up >= n)	// if 硈尿 n 2 retry count0, 玥秸糴WiFi duration
			{
				WaitCount = 0; 
				n = 3;
				up = 0;
		...
		else if (retryCount <= 3)	// <=3 retry in the last 2-second duration
		{
			...
			if (dn == 2)	// if 硈尿 2 2 retry count< 3, 玥秸WiFi duration
			{
				if (WaitCount <= 2)
					m++; // 磷ㄢlevelいㄓ
				else
				...
				if ( m >= 20) //m 程 = 20 ' 程120 recheck琌秸俱 WiFi duration.
					m = 20;
		...
		else  //retry count > 3, 璶1Ω retry count > 3, 玥秸WiFi duration
		{
			if (WaitCount == 1)
				m++; // 磷ㄢlevelいㄓ
			else
