...
)
{
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		DHD_DBG_PM("dev_wlc_ioctl(cmd:WLC_SET_PM  power_mode:%d)\r\n", pm_local);
...
#if 0 // terence 20150317: fix for scan results list order by rssi issue in wpa_supplicant
...
)
{
	...
	WL_TRACE(("\n%s, cmd:%x called via dhd->dhd_ioctl_entry()\n", __FUNCTION__, cmd));
	if (cmd < SIOCIWFIRST ||
	...
	case WLC_E_ROAM:
		if (status != WLC_E_STATUS_SUCCESS) {
			...
			if ((roam_no_success == 3) && (roam_no_success_send == FALSE)) {
				// terence 20150317: fix roaming issue
