...
#if defined(WL_WIRELESS_EXT)
module_param(iw_msg_level, int, 0644);
#endif
#ifdef WL_CFG80211
module_param(wl_dbg_level, int, 0644);
#endif
...
/* Watchdog interval */
uint dhd_watchdog_ms = 1000; //by Peter...
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
			...
			DHD_DBG_PM("dhd_wl_ioctl_cmd(cmd:WLC_SET_PM  power_mode:%d)\r\n", power_mode);
			...
			dhd_wl_ioctl_cmd(dhd, WLC_SET_PM, (char *)&power_mode,
				sizeof(power_mode), TRUE, 0);
			DHD_DBG_PM("dhd_wl_ioctl_cmd(cmd:WLC_SET_PM  power_mode:PM_FAST)\r\n");
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	DHD_OS_WAKE_LOCK(&dhd->pub);
	DHD_DBG_PM("net->name %s cmd 0x%04x\r\n", net->name, cmd);
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if ((firmware_path != NULL) && (firmware_path[0] != '\0'))
		...
		dhd_conf_set_fw_name_by_chip(&dhd->pub, fw_path, fw_path);
