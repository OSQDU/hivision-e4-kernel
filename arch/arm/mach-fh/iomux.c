...
#define CONFIG_FH_SPI1 /*test driving spi nand flash*/
#ifdef CONFIG_FH_SPI1
# define IO_SD1_SPI1	1
#else /* SD1 */
# define IO_SD1_SPI1	0
#endif
	{
		.func_name = { "SD1_CLK", "SSI1_CLK", "", "", },
		.reg_type  = 8,
		.func_sel  = IO_SD1_SPI1,
		.pupd 	   = IOMUX_PUPD_NONE,
		.drv_cur   = 1,
	},
	{
...
static void fh81_iomux_setmfs(Iomux_Object *iomux_obj, Iomux_Pad *pad)
{
	switch (pad->reg_type) {
	case 8:
...
static int fh81_iomux_getmfs(Iomux_Object *iomux_obj, Iomux_Pad *pad)
{
	int mfs;
...
static void fh81_iomux_print(Iomux_Object *iomux_obj)
{
	int i;
	u32 reg;
...
static void fh81_iomux_setcur(Iomux_Object *iomux_obj, Iomux_Pad *pad)
{
	switch (pad->reg_type) {
	case 5:
...
static void fh81_iomux_setpupd(Iomux_Object *iomux_obj, Iomux_Pad *pad)
{
	switch (pad->reg_type) {
	case 9:
...
static void fh81_iomux_setrest(Iomux_Object *iomux_obj, Iomux_Pad *pad)
{
	switch (pad->reg_type) {
	case 5:
...
void fh81_iomux_init(Iomux_Object *iomux_obj)
{
	int i;
	u32 reg = 0;

	iomux_obj->pads = fh81_iomux_cfg;

	for (i = 0; i < ARRAY_SIZE(fh81_iomux_cfg); i++) {
		iomux_obj->pads[i].reg_offset = i * 4;
		iomux_obj->pads[i].reg = &reg;
		fh81_iomux_setmfs(iomux_obj, &fh81_iomux_cfg[i]);
		fh81_iomux_setcur(iomux_obj, &fh81_iomux_cfg[i]);
		fh81_iomux_setpupd(iomux_obj, &fh81_iomux_cfg[i]);
		fh81_iomux_setrest(iomux_obj, &fh81_iomux_cfg[i]);
	...
	reg = fh_pmu_get_reg(REG_PMU_MAC_REF_CLK_CFG);
	reg |= (1 << 13);
	fh_pmu_set_reg(REG_PMU_MAC_REF_CLK_CFG, reg);
#else
