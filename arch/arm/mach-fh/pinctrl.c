...
#include <mach/iopad_qfn.h>
#else
#include <mach/iopad_bga.h>
#endif
...
static void fh_pinctrl_init_pin(void)
{
    ...
    for(i=0; i<PAD_NUM; i++)
    {
        ...
        if(pin->reg->bit.pdn == PUPD_NONE)
            if(pin->reg->bit.pun == PUPD_NONE)
                pin->pullup_pulldown = PUPD_NONE;
            else
                pin->pullup_pulldown = PUPD_UP;
        else
            pin->pullup_pulldown = PUPD_DOWN;
    }
}
...
void fh_pinctrl_prt(struct seq_file *sfile)
{
    ...
    for(i=0; i<PAD_NUM; i++)
    {
        ...
        seq_printf(sfile, "%02d\t%8s\t0x%04x\t0x%08x\t%04d\t%04d\t%04d\t%04d\n",
                ...
                pinctrl_obj.pinlist[i]->reg_offset,
